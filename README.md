...
2. Create a new webhook (e.g., "Spotify Bot")
3. Copy the webhook URL (looks like `https://discord.com/api/webhooks/...`)

**Step 2: Configure `.env.discord`**
```bash
echo "DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/YOUR_WEBHOOK_URL" > .env.discord
```
...
requests.post(webhook, json={'content': '✅ Spotify pipeline connected!'})
"
```
...
**`.env.spotify`** (auto-generated by `hermes auth spotify`, never committed)
```bash
...
- **OAuth cache:** `${HOME}/.cache/spotify_oauth`
