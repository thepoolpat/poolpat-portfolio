...
class TestFetchSpotifyAll(unittest.TestCase):
    ...
    @patch.dict(os.environ, {"SPOTIFY_REFRESH_TOKEN": "rt", "SPOTIFY_CLIENT_ID": "cid"}, clear=True)
    @patch("fetch_plays._get_spotify_token", return_value=None)
    @patch("spotify_client.SpotifyClient")
    @patch("spotify_auth.refresh_access_token", return_value={"access_token": "live_token"})
    def test_user_pkce_path_taken_with_refresh_token(self, mock_refresh, mock_client_cls, _tok):
        """With SPOTIFY_REFRESH_TOKEN + SPOTIFY_CLIENT_ID set, the user-scoped PKCE
        path runs: refresh the access token, build a SpotifyClient, pull top tracks
        and recently-played, and stamp auth_method='user_token'. The public token is
        forced to None so no catalog network call happens — user data alone is enough."""
        client = MagicMock()
        mock_client_cls.return_value = client
        client.get_current_user.return_value = {"display_name": "Poolpat", "product": "premium"}
        client.get_top_tracks.side_effect = lambda time_range, limit: [
            {"name": f"{time_range} hit", "popularity": 60},
        ]
        client.get_recently_played.return_value = [
            {"track": {"name": "Recent One"}, "played_at": "2026-01-01T00:00:00Z"},
        ]

        existing = {"tracks": {"Old": 10}, "total_streams": 12345}
        sp_data, got = fetch_plays.fetch_spotify_all(existing)

        # The user/PKCE branch was actually exercised.
        mock_refresh.assert_called_once_with("cid", "rt")
        client.get_top_tracks.assert_any_call("short_term", 50)
        client.get_top_tracks.assert_any_call("medium_term", 50)
        client.get_top_tracks.assert_any_call("long_term", 50)
        client.get_recently_played.assert_called_once_with(50)

        # User auth succeeded → got_data True even though the public token is None.
        self.assertTrue(got)
        self.assertEqual(sp_data["fetch_status"], "success")
        self.assertEqual(sp_data["auth_method"], "user_token")
        # Top-track names surface only on the user path.
        self.assertEqual(sp_data["top_tracks_short"], ["short_term hit"])
        self.assertEqual(sp_data["top_tracks_long"], ["long_term hit"])
        # Manually-entered total_streams is still preserved.
        self.assertEqual(sp_data["total_streams"], 12345)
    ...
    @patch("spotify_client.SpotifyClient")
    @patch("spotify_auth.refresh_access_token", return_value={"access_token": "live_token"})
    def test_user_auth_failure_falls_back_to_public_api(self, mock_refresh, mock_client_cls, _tok, mock_get):
        """If the user-scoped client raises (expired refresh token, network blip), the
        documented fallback is to swallow the error and use the public Client
        Credentials API — auth_method ends up 'client_credentials', not a crash."""
        client = MagicMock()
        mock_client_cls.return_value = client
        client.get_current_user.side_effect = Exception("refresh rejected")

        # Public catalog path: album-group sweep → album batch → track batch.
        album_resp = _resp(200, {"items": [{"id": "alb1"}], "next": None})
        empty = _resp(200, {"items": [], "next": None})
        album_batch_resp = _resp(200, {"albums": [{"tracks": {"items": [
            {"id": "t1", "artists": [{"id": fetch_plays.SPOTIFY_ARTIST_ID}]},
        ]}}]})
        track_batch_resp = _resp(200, {"tracks": [{"name": "Public Song", "popularity": 40}]})
        mock_get.side_effect = [album_resp, empty, empty, empty, album_batch_resp, track_batch_resp]

        sp_data, got = fetch_plays.fetch_spotify_all({"tracks": {}, "total_streams": 0})

        # The user path was attempted then abandoned for the public fallback.
        mock_refresh.assert_called_once_with("cid", "rt")
        self.assertTrue(got)
        self.assertEqual(sp_data["fetch_status"], "success")
        self.assertEqual(sp_data["auth_method"], "client_credentials")
        # Fallback produced catalog popularity, and no user-only keys leaked in.
        self.assertEqual(sp_data["tracks"]["Public Song"], 40)
        self.assertNotIn("top_tracks_short", sp_data)
...
# ─── load_existing_data (monotonic-baseline guard) ───────────────────────────

class TestLoadExistingData(unittest.TestCase):
    """A corrupt plays.json must abort loudly, NOT silently start from {} —
    starting empty would let save_plays_json() overwrite the (recoverable)
    file and zero the monotonic baseline. This codifies that deliberate
    raise so it can't be 'fixed' into a silent fallback."""
    ...
    def setUp(self):
        ...
        self._dir = tempfile.TemporaryDirectory()
        self.addCleanup(self._dir.cleanup)
        self.plays = Path(self._dir.name) / "plays.json"
        patcher = patch.object(fetch_plays, "PLAYS_JSON", self.plays)
        patcher.start()
        self.addCleanup(patcher.stop)

    def test_missing_file_returns_empty(self):
        self.assertEqual(fetch_plays.load_existing_data(), {})

    def test_valid_file_round_trips(self):
        self.plays.write_text('{"soundcloud": {"total_plays": 5000}}')
        self.assertEqual(
            fetch_plays.load_existing_data(),
            {"soundcloud": {"total_plays": 5000}},
        )

    def test_corrupt_file_raises_not_silent_empty(self):
        self.plays.write_text('{"soundcloud": {"tracks": ')  # truncated
